...
// 将容器转化为数组，返回的数组包含容器的所有元素
Object[] toArray();

// 添加元素e，成功返回true，失败返回false
boolean add(E e);

// 删除元素o，成功返回true，失败返回false
boolean remove(Object o);

// 添加 c 的所有元素到当前容器中，集合并运算
boolean addAll(Collection<? extends E> c);

// 在当前容器中删除包含在 c 中的元素，集合差运算
...
// 在 index 位置有序插入 c 中的所有元素
boolean addAll(int index, Collection<? extends E> c);

// 删除指定位置的元素
E remove(int index);

// 返回 元素o 首次出现的位置，没有返回 -1
int indexOf(Object o);

// 返回元素o 最后出现的位置
int lastIndexOf(Object o);

// 返回从 index 位置开始的列表迭代器
...
// 创建初始容量为 initialCapacity 的空数组列表
public ArrayList(int initialCapacity)

// 创建包含容器 c 所有元素的数组列表，元素次序与 c 同
...
```java
// 判断是否还有后继元素，有返回 true，没有返回 false
boolean hasNext();
...
// 返回基于 previous() 调用的元素序号
int previousIndex();

// 用元素 e 替换列表当前的元素
void set(E e);

// 将 e 插入列表中，next()方法返回的元素之前，previous() 方法返回的元素之后，若是空列表，直接插入
void add(E e);
```
